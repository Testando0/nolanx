...
// PEGAR LINK MIDIAS(QU.AX)
async function upload(midia) {
try {
const { ext } = await fromBuffer(midia);
const form = new FormData();
form.append("files[]", midia, { filename: `tmp.${ext}` });
...
throw new Error("Upload falhou: " + JSON.stringify(uploadJson));
}
return uploadJson.files[0].url;
} catch (erro) {
console.error("Erro upload QU.AX:", erro.message);
throw erro;
}
}
...
const banner3 = cfonts.render(("NOLAN"), {
  font: 'block',
  align: "center",
  gradient: ["red", "magenta"]
});
...
const authorname = "Nolan Multi-Device"
const packname = "Criador: @euzenox"

const chyt = "558892153207@s.whatsapp.net";
const nit = "558892153207@s.whatsapp.net";
const supre = "558892153207@s.whatsapp.net";
